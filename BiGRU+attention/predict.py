...
import codecs
import time
from model_server import BiGRUAttention

#configuration
FLAGS=tf.app.flags.FLAGS
tf.app.flags.DEFINE_integer("num_classes",4875,"number of label")
        ...
        with tf.Session(config=config) as sess:
            ...
            for start, end in zip(range(0, number_of_training_data, FLAGS.batch_size),range(FLAGS.batch_size, number_of_training_data+1, FLAGS.batch_size)):
                if FLAGS.char_attention_flag:
                    ...
                    attention, predictions=sess.run([model.attention,model.predictions],feed_dict={model.input_x:testX[start:end],model.dropout_keep_prob:1})
                lines_sublist=lines[start:end]
                get_label_using_logits_batch(lines_sublist, attention, predictions, vocabulary_index2word_label, predict_target_file_f,FLAGS.threshold, label_name)
            t1 = time.time()
...
def get_label_using_logits_batch(lines_sublist,attention, predictions_batch,vocabulary_index2word_label,f,threshold, label_name):
    for i,predictions in enumerate(predictions_batch):
        ...
        write_question_id_with_labels(lines_sublist[i], attention[i], label_list, f, label_names)
    f.flush()

def write_question_id_with_labels(question_id,attention, labels_list,f, label_names):
    labels_string=",".join(labels_list)
    label_string_name = ','.join(label_names)
    attention_list = ','.join(map(str, attention))
    f.write(question_id+"\t"+labels_string+ '\t' + label_string_name + "\t"+attention_list+"\n")
