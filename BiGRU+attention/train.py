...
tf.app.flags.DEFINE_integer("num_classes",5730,"number of label")
...
tf.app.flags.DEFINE_integer("num_epochs",9,"number of epochs to run.")
...
tf.app.flags.DEFINE_integer("train_sample_num",10000,"train sample num")
tf.app.flags.DEFINE_integer("dev_sample_num",10000,"dev sample num")
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        model = BiGRUAttention( FLAGS.loss_number, FLAGS.num_count, FLAGS.num_classes, FLAGS.learning_rate, FLAGS.batch_size, FLAGS.decay_steps,
            ...
            for epoch in range(curr_epoch,FLAGS.num_epochs):
                ...
                while train_example_num < train_sample_num:
                    try :
                        ...
                        summary_merge, global_step, curr_loss,curr_acc, curr_f1, curr_recall, curr_precision, _=sess.run([model.summary_merge, model.global_step, model.loss_val,model.accuracy,model.f1,model.recall,model.precision,model.train_op],feed_dict)
                        ...
                        if counter %2000==0:
                            print("Epoch %d\tBatch %d\tTrain Loss:%.3f\tTrain Accuracy:%.3f\tTrain precision:%.3f\tTrain recall:%.3f\tTrain f1:%.3f" %(epoch,counter,loss/float(counter),acc/float(counter),precision/float(counter),recall/float(counter),f1/float(counter)))
                ...
                while dev_example_num < dev_sample_num:
                    ...
                    try:
                        ...
                        if FLAGS.char_attention_flag:
                            ...
                            testXChar=[]
                        cur_eval_loss, cur_eval_acc, cur_eval_precision, cur_eval_recall, cur_eval_f1 = do_eval(dev_writer, sess, model, testX, testXChar, testY)
                        ...
                        if counter_dev % FLAGS.validate_step == 0:
                            print("Epoch %d \tBatch %d\tValidation Loss:%.3f\tValidation Accuracy: %.3f\tValidation precision:%.3f\tValidation recall:%.3f\tValidation f1:%.3f\t" % (epoch,counter_dev,eval_loss/float(counter_dev),eval_acc/float(counter_dev),eval_precision/float(counter_dev),eval_recall/float(counter_dev),eval_f1/float(counter_dev)))
...
# validate 
def do_eval(dev_writer, sess,model,evalX,evalXChar,evalY):
    if FLAGS.char_attention_flag:
    ...
    dev_summary_merge, global_step, curr_eval_precision, curr_eval_recall, curr_eval_f1, curr_eval_loss, logits,curr_eval_acc= sess.run([model.summary_merge, model.global_step, model.precision, model.recall, model.f1, model.loss_val,model.logits,model.accuracy],feed_dict)
    dev_writer.add_summary(dev_summary_merge, global_step)
    return curr_eval_loss, curr_eval_acc, curr_eval_precision, curr_eval_recall, curr_eval_f1
