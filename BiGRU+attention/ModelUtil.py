...
def extract_parameter(modelPath):
    ...
    saver = tf.train.import_meta_graph(os.path.join(modelPath, "model.ckpt-7.meta"))
    config = tf.ConfigProto(allow_soft_placement=True)
    with tf.Session(config=config) as sess:
        with tf.device("/cpu:0"):
            saver.restore(sess, os.path.join(modelPath, "model.ckpt-7"))
...
def save_pkl(modelPath):
    parames = dict()
    sum = 0
    saver = tf.train.import_meta_graph(os.path.join(modelPath, "model_ckpt-7.meta"))
    config = tf.ConfigProto(allow_soft_placement=True)
    with tf.Session(config=config) as sess:
...
if __name__ == "__main__":
    extract_parameter("./checkpoint/HAN_300d")
