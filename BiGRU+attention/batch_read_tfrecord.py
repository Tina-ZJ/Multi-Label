...
class SegBatcher(object):
    ...
    def example_parser(self, filename_queue):
        ...
        return terms, chars, labels

    def input_pipeline(self, filenames, batch_size,  num_epochs=None):
        filename_queue = tf.train.string_input_producer([filenames], num_epochs=num_epochs, shuffle=True)
        terms, chars, labels = self.example_parser(filename_queue)
        min_after_dequeue = 10000
        capacity = min_after_dequeue + 12 * batch_size
        print ('starting to begaining')
        next_batch = tf.train.batch([terms, chars, labels], batch_size=batch_size, capacity=capacity, dynamic_pad=True, allow_smaller_final_batch=True)
        print ('ending to .......')
        return next_batch
